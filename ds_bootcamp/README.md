...
<details>
<summary><strong>AI1: Твой личный ИИ-агент</strong></summary>

*   **Тема:** Создание персонального ИИ-агента на базе LLM для сопровождения образовательного пути в «Школе 21»: от выбора специализации до подбора учебных материалов и анализа рынка труда.
*   **Задания:**
    *   **Задание 1. Знакомство с инструментами:** развёртывание `n8n` в облаке `Cloud.ru` через Docker; подключение к `GigaChat API` через платформу Foundation Models; сборка первого воркфлоу и получение ответа от LLM.
    *   **Задание 2. Выбор специализации:** создание Telegram-бота с AI-агентом на `GigaChat 2` и памятью (`SimpleMemory`); бот задаёт пользователю серию вопросов (опыт, интересы, цели) и выдаёт персональную рекомендацию по специализации; написание системного промпта для управления поведением агента.
    *   **Задание 3. Дайджест новостей:** парсинг постов из Telegram-каналов по специализации через RSS; автоматическая суммаризация и ежедневная отправка дайджеста в приватный Telegram-канал с помощью `GigaChat 2`.
    *   **Задание 4. Анализ вакансий:** еженедельный парсинг вакансий через `HH.ru API`; извлечение топ-15 ключевых навыков и требований с помощью AI-агента; отправка отчёта в Telegram-канал.
    *   **Задание 5. Персональный ментор:** бот формирует персональный контент-план по заданной теме, ищет материалы через `Tavily Search` и отправляет обучающие посты без повторов; прогресс хранится в `Static Data` внутри n8n.
    *   **Задание 6 (бонусное):** работа с методом RAG для подключения внешней базы знаний к агенту.
...
-   **Веб-парсинг:** Requests, BeautifulSoup, HH.ru API, RSS
